...
public class history_report extends Activity{
 ...
 //       		test.setText(strResult);
       ...
       helper = new DatabaseHelper(this, DatabaseHelper.DATABASE_NAME, null,DatabaseHelper.Version);     
	   db = helper.getWritableDatabase();//打开数据库
	   c=db.query(DatabaseHelper.HISTORY_REPORT, null, DatabaseHelper.USER_ID+"="+userid, null, null, null,DatabaseHelper.ID+" desc",null);//查询银行名字
	   Log.v("inqian", "in11");    
	   c.moveToNext();
	   if(!c.isAfterLast()){
	   ...
	   db.close();
		 ...
		 /*   ListAdapter adapter = new SimpleCursorAdapter(this, 
	        ...
	        //响应选中某一银行事件，启动item_details Activity
	...
	public void setlinear(Context context,int id,ImageView icon){
		...
		Cursor c=db.query(DatabaseHelper.HISTORY_REPORT, null, DatabaseHelper.ID+"="+id, null, null, null,DatabaseHelper.ID+" desc",null);//查询银行名字				        
        c.moveToFirst();
        ContentValues values = new ContentValues();      
        values.put(DatabaseHelper.HISTORY_REPORT_TAG, 1);
