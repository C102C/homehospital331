...
public class PC300 {
	...
	/**
	 * 将收到的多条数据分割，分割标志位包头的两个字节0xaa 0x55
	 * 
	 * @param buffer
	 * @return
	 */
	public List<byte[]> splitBufferData(byte[] buffer) {
		List<byte[]> datas = new ArrayList<byte[]>();
		// 每条独立的数据的前五个字节大小是固定的,最短的数据为6个字节
		int low = 0, high = buffer.length;// 初始假设只有一条数据
		byte head0 = (byte) 0xaa;
		byte head1 = 0x55;
		for (int i = 5; i < buffer.length; i++) {
			if (buffer[i - 1] == head0 && buffer[i] == head1) {
				high = i - 1;
	...
	public SparseArray<byte[]> getSingleLegalPatternFromBuffer(byte[] buffer) {
		return checkCrcAndRetainSinglePattern(splitBufferData(buffer));
	}
	...
	public SparseArray<List<byte[]>> getLegalPatternsFromBuffer(byte[] buffer) {
		List<byte[]> datas = splitBufferData(buffer);
	...
	public static class PersistWriter extends Thread {
		private static PersistWriter instance = null;// 单例模式
		private BluetoothService bluetoothService;
		private static final String TAG = "PC300.PersistWriter";

		public PersistWriter(BluetoothService bluetoothService) {
			this.bluetoothService = bluetoothService;
		}

		public static PersistWriter getInstance(
				BluetoothService bluetoothService) {
			if (instance == null)
				instance = new PersistWriter(bluetoothService);
			return instance;
		}
		...
		public void run() {
			while (true) {
				if (bluetoothService.getState() == BluetoothService.STATE_CONNECTED) {
					Log.i(TAG, "send bettery command");
					bluetoothService.write(COMMAND_BETTERY);// 电量命令
				}
				try {
					TimeUnit.MILLISECONDS.sleep(3000);// 每隔三秒发送一次电量查询命令
